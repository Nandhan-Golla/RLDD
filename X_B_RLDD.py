...
class SimulatedPatientData:
    def __init__(self, num_patients=100):
        ...
        # Drug effects: [delta_hemoglobin, delta_glucose, side_effect_severity] for 20 drugs
...
if __name__ == "__main__":
    ...
    new_blood_report = "Patient Test: Hemoglobin: 11.5 g/dL, Glucose: 130.0 mg/dL, Age: 45, Weight: 70.0 kg"
    new_protein_scan = "test_protein_scan.jpg"
    pipeline = DataPipeline()
    blood_features = pipeline.process_blood_report(new_blood_report)
    hemoglobin = float(re.search(r"Hemoglobin:\s*(\d+\.\d+)", new_blood_report).group(1))
    glucose = float(re.search(r"Glucose:\s*(\d+\.\d+)", new_blood_report).group(1))
    age = float(re.search(r"Age:\s*(\d+)", new_blood_report).group(1))
    weight = float(re.search(r"Weight:\s*(\d+\.\d+)", new_blood_report).group(1))
